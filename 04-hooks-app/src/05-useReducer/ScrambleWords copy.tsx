...
export const ScrambleWords = () => {
    ...
    const handleGuessSubmit = (e: React.FormEvent) => {
        ...
        if (guess !== currentWord) {
            ...
            const currentErrorCounter = errorCounter + 1;

            setErrorCounter(current => currentErrorCounter <= maxAllowErrors ? currentErrorCounter : current);
            setIsGameOver(currentErrorCounter >= maxAllowErrors);
            setGuess('');
            return;
        }


        const newCurrentWords = words.filter(word => word !== currentWord);

        const currentPoints = points + 1;
        ...
        setGuess('');
        setWords(newCurrentWords);
        setCurrentWord(newCurrentWords[0]);
        setScrambledWord(scrambleWord(newCurrentWords[0]));
        setIsGameOver(newCurrentWords.length === 0);
        setErrorCounter(0);
    ...
    const handleSkip = () => {
        ...
        if (maxSkips <= skipCounter) {
            return
        }

        const currentSkipCounter = skipCounter + 1;
        const updatedWords = words.filter(word => word !== currentWord);

        setSkipCounter(current => currentSkipCounter < maxSkips ? currentSkipCounter : current);
        setWords(updatedWords);
        setCurrentWord(updatedWords[0]);
        setScrambledWord(scrambleWord(updatedWords[0]));
        setIsGameOver(currentSkipCounter >= maxSkips);
        setGuess('');
