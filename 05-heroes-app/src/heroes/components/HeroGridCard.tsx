...
export const HeroGridCard = ({ hero }: IProps) => {
        ...
        <Card className="group overflow-hidden hover:shadow-xl transition-all duration-300 hover:-translate-y-1 bg-gradient-to-br from-white to-gray-50">
            <div className="relative h-64">
                <img
                    src={hero.image}
                    alt={hero.name}
                    className="object-cover transition-all duration-500 group-hover:scale-110 absolute top-[-30px] w-full h-[400px]"
                    onClick={onHandleClick}
                />

                {/* Status indicator */}
                <div className="absolute top-3 left-3 flex items-center gap-2">
                    <div className={`w-3 h-3 rounded-full ${hero.status === "Active" ? "bg-blue-500" : "bg-gray-500"}`} />
                ...
                <Button size="sm" variant="ghost" className="absolute bottom-3 right-3 bg-white/90 hover:bg-white" onClick={() => toggleFavorite(hero)}>
                    <Heart className={`h-4 w-4 ${ isFavorite(hero) ? "fill-red-500 text-red-600" : "text-gray-600" }`} />
                </Button>
            ...
            <CardContent className="space-y-4">
                ...
                <div className="grid grid-cols-2 gap-3">
                    <div className="space-y-1">
                        ...
                        <Progress value={hero.strength} className="h-2" activeColor="bg-orange-500" />
                    </div>
                    <div className="space-y-1">
                        <div className="flex items-center gap-1">
                        ...
                        <Progress value={hero.intelligence} className="h-2" activeColor="bg-blue-500" />
                    </div>
                    <div className="space-y-1">
                        <div className="flex items-center gap-1">
                        ...
                        <Progress value={hero.speed} className="h-2" activeColor="bg-green-500" />
                    </div>
                    <div className="space-y-1">
                        <div className="flex items-center gap-1">
                        ...
                        <Progress value={hero.durability} className="h-2" activeColor="bg-purple-500" />
                    </div>
                </div>
